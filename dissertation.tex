...
\cosupervisor{Andrey Mokhov (Newcastle University)}
...
n \arrow[u, "A"] & n \times m \arrow[u, "A \otimes B" description] & m \arrow[u, "B"']
    ...
    \section{Probabilistic Interpretation of Selective Functors}
        ...
        Note that a \texttt{toArrow} function is required to type check in the listing \ref{lst:safm}. An equivalent \texttt{toMatrix} function would be necessary in order to match the same signature but, unfortunatly an \texttt{Enum (a $\rightarrow$ b)} instance would be necessary and that is impossible to achieve in Haskell at the moment. Because of this, the signature of \texttt{select} has been adapted.
...
A + A &  & {A\ =\ [\frac{\Phi_p}{\Phi_{-p}}]} \arrow[ll, "p?"']
        ...
        Looking closely at the diagram one can see some resemblance to what one gets from definition \ref{lst:safm2}, and indeed McCarthy's conditionals exploit the same conditional nature that is intrinsic to \gls{saf}.
...
corr p = let f = fromF p :: Matrix e a ()
        ...
        A \gls{saf} can be seen equivalent to a Monad, if the associated data type is \texttt{(Enum a, Bounded a, Eq a)}. See for instance:
...
class Selective m => Monad m where
  ...
  (>>=) ma famb =
    ...
    \section{Probabilistic Programming eDSL}\label{prob-edsl}
    ...
    While the current work has made it possible to find a probabilistic interpretation for \glspl{saf} in the light of the Arrow abstraction and the \texttt{select} operator, it didn't made it possible to benefit from any of \gls{saf}' properties. 
...
>
    ...
    Lets look at an example that cannot be express by using our \gls{laop} matrix library:
    
    \begin{lstlisting}[language=Haskell, caption={Throw N coins},captionpos=b]
-- | Throw @n@ coins
throw :: Int -> Dist [Coin]
throw 0 = pure []
throw n =
...
{-
Result:
> runToIO $ sample (prog 10) 10
[[Tails,Tails,Heads],[Heads],[Heads],[Tails,Tails,Heads],[Tails,Heads],[Heads],[Heads],[Heads],[Tails,Heads],[Heads]]
>
-}
    \end{lstlisting}
...
[12,18,12,11,9,9,8,9,18,6,6,18,15,6,9,17,6,9,18,12]
>
-}
    \end{lstlisting}
...
-}
    ...
    We can see that the \texttt{toss} effect is being repeated because we do not have a way to forward its conditional result to the next computation and this leads to the program not behaving as expected. Given this, an interesting observation, regarding McCarthy's Conditional, is the fact that if we allow every value through the computation we retrieve an approximate version of the monadic bind function:
    
    \begin{lstlisting}[language=Haskell, caption={Partial monadic bind function},captionpos=b]
almostBind :: Selective f => f b -> f (b -> c) -> f c
almostBind = flip (condS (pure (const True)) undefined) -- undefined is never called
    \end{lstlisting}
...
rejection predicate observed proposal model = loop where
  ...
  loop =
    ...
    This method and more complex others require monadic capabilities which make the proposed solution quite inefficient. This seems to be a limitation of the selective abstraction. Although it is still possible to implement such algorithms, the tradeoff between performance and expressivity does not seem to be worth it. 
    
    \subsection{Static analysis of probabilistic programs}
    
    **Mencionar os benefícios à priori de usar a eDSL com capacidades de análise estática \cite{gordon2014probabilistic}**
...
-}
    ...
    As can be seen in the plot of Figure \ref{fig:bench}, the \emph{hmatrix} and \emph{matrix} libraries are those that perform better. By observing their internal structure, one realises that they are a suitable representation for BLAS/LAPACK computations \citep{anderson1999lapack}, that is, they have been designed to efficiently exploit caches on modern cache-based architectures. A matrix in the \emph{linear} library is defined as \texttt{Vector cols (Vector rows Double)} and does not take into account cache lengths or sizes, so it behaves much worse than the previous ones. Our structure does not take into account any low-level optimisations either, being unable to compete with those that do. Nevertheless, the implementation is \emph{performant for a cache-oblivious approach} and behaves better (almost one order of magnitude better) than other types of simpler definitions.
        ...
        \subsection{Distribution matrix vs distribution monad}
        
            \subsubsection{Applicative vs Selective}
